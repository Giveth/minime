...
describe('MiniMeToken test', () => {
    ...
    before(async () => {
        testrpc = TestRPC.server({
            ws: true,
            gasLimit: 5800000,
            //logger: { log: console.log }, // Uncomment this line to get rpc trasactions in logs.
            total_accounts: 10,
        });
    ...
    it('Should allow the controller to generate tokens and add it to controller\'s address.',
    async () => {
        b[0] = await web3.eth.getBlockNumber();
        await miniMeToken.generateTokens(10, {from: accounts[1]});
